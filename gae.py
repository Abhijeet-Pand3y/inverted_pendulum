...
def compute_gae(rewards, values, next_values, dones, gamma=0.975, lam=0.95):
    ...
    for t in range(len(rewards) - 1, -1, -1):
        not_done = 1.0 - dones[t, 0].astype(np.float32)

        delta = rewards[t, 0] + gamma * next_values[t, 0] * not_done - values[t, 0]
...
def train_advantage_vpg(
    ...
    for x in range(epochs):
        ...
        for _ in range(critic_updates):
            ...
            critic_loss = mse_loss(critic(states_t), rtg_t)
        ...
        for _ in range(updates):
            ...
            actions_t = 2 * (actions_t - amin) / (amax - amin) - 1
